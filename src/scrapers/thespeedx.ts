// ─── TheSpeedX Fetcher ────────────────────────────────────────────────────────
// Fetches proxies from TheSpeedX PROXY-List GitHub raw files.

import axios from 'axios';
import type { RawProxy, ProxyProtocol } from '../types.js';
import { createLogger } from '../utils/logger.js';

const log = createLogger('scraper:thespeedx');

const BASE = 'https://raw.githubusercontent.com/TheSpeedX/PROXY-List/master';
  ...
  for (const { url, protocol } of SOURCES) {
    try {
      ...
      for (const line of lines) {
        ...
        results.push({ host: host.toLowerCase().trim(), port, protocol, source: 'thespeedx' });
      }
    } catch (err) {
