// ─── Roosterkid Fetcher ───────────────────────────────────────────────────────
// Hourly updates. ~60-200 per file.

import axios from 'axios';
import type { RawProxy, ProxyProtocol } from '../types.js';
import { createLogger } from '../utils/logger.js';

const log = createLogger('scraper:roosterkid');
const BASE = 'https://raw.githubusercontent.com/roosterkid/openproxylist/main';
const SOURCES: Array<{ url: string; protocol: ProxyProtocol }> = [
  { url: `${BASE}/HTTPS_RAW.txt`, protocol: 'http' },
  { url: `${BASE}/SOCKS4_RAW.txt`, protocol: 'socks4' },
  { url: `${BASE}/SOCKS5_RAW.txt`, protocol: 'socks5' },
];
  ...
  for (const { url, protocol } of SOURCES) {
    try {
      ...
      for (const line of (res.data as string).split('\n').map(l => l.trim()).filter(l => l && !l.startsWith('#'))) {
        ...
        if (host && port && !isNaN(port)) results.push({ host: host.toLowerCase().trim(), port, protocol, source: 'roosterkid' });
      }
