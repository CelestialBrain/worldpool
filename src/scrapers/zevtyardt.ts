// ─── Zevtyardt Fetcher ─────────────────────────────────────────────────────────
// ~8-10k proxies per file.

import axios from 'axios';
import type { RawProxy, ProxyProtocol } from '../types.js';
import { createLogger } from '../utils/logger.js';

const log = createLogger('scraper:zevtyardt');
const BASE = 'https://raw.githubusercontent.com/zevtyardt/proxy-list/main';
  ...
  for (const { url, protocol } of SOURCES) {
    try {
      ...
      for (const line of (res.data as string).split('\n').map(l => l.trim()).filter(l => l && !l.startsWith('#'))) {
        ...
        if (host && port && !isNaN(port)) results.push({ host: host.toLowerCase().trim(), port, protocol, source: 'zevtyardt' });
      }
