// ─── ProxyScraper GitHub Fetcher ───────────────────────────────────────────────
// Updated every 30 minutes. ~5k+ per file.

import axios from 'axios';
import type { RawProxy, ProxyProtocol } from '../types.js';
import { createLogger } from '../utils/logger.js';

const log = createLogger('scraper:proxyscraper-gh');
const BASE = 'https://raw.githubusercontent.com/ProxyScraper/ProxyScraper/main';
  ...
  for (const { url, protocol } of SOURCES) {
    try {
      ...
      for (const line of (res.data as string).split('\n').map(l => l.trim()).filter(l => l && !l.startsWith('#'))) {
        ...
        if (host && port && !isNaN(port)) results.push({ host: host.toLowerCase().trim(), port, protocol, source: 'proxyscraper-gh' });
      }
