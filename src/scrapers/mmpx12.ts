// ─── Mmpx12 Fetcher ───────────────────────────────────────────────────────────
// ~1k proxies, hourly updates, 44k+ commits.

import axios from 'axios';
import type { RawProxy, ProxyProtocol } from '../types.js';
import { createLogger } from '../utils/logger.js';

const log = createLogger('scraper:mmpx12');

const BASE = 'https://raw.githubusercontent.com/mmpx12/proxy-list/master';
  ...
  for (const { url, protocol } of SOURCES) {
    try {
      ...
      for (const line of lines) {
        ...
        results.push({ host: host.toLowerCase().trim(), port, protocol, source: 'mmpx12' });
      }
    } catch (err) {
