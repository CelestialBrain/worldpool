// ─── Iplocate Fetcher ─────────────────────────────────────────────────────────
// ~8k+ proxies, validated every 30 minutes. Plain host:port per protocol file.

import axios from 'axios';
import type { RawProxy, ProxyProtocol } from '../types.js';
import { createLogger } from '../utils/logger.js';

const log = createLogger('scraper:iplocate');

const BASE = 'https://raw.githubusercontent.com/iplocate/free-proxy-list/main/protocols';
  ...
  for (const { url, protocol } of SOURCES) {
    try {
      ...
      for (const line of lines) {
        ...
        results.push({ host: host.toLowerCase().trim(), port, protocol, source: 'iplocate' });
      }
    } catch (err) {
