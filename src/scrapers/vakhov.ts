// ─── Vakhov Fetcher ───────────────────────────────────────────────────────────
// ~900 proxies, updated every 5-20 minutes. Served via GitHub Pages.

import axios from 'axios';
import type { RawProxy, ProxyProtocol } from '../types.js';
import { createLogger } from '../utils/logger.js';

const log = createLogger('scraper:vakhov');

const BASE = 'https://vakhov.github.io/fresh-proxy-list';
  ...
  for (const { url, protocol } of SOURCES) {
    try {
      ...
      for (const line of lines) {
        ...
        results.push({ host: host.toLowerCase().trim(), port, protocol, source: 'vakhov' });
      }
    } catch (err) {
