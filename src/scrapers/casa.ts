// ─── Casa-ls Fetcher ──────────────────────────────────────────────────────────
// ~5k+ proxies, updated every 5 minutes. Files have no .txt extension.

import axios from 'axios';
import type { RawProxy, ProxyProtocol } from '../types.js';
import { createLogger } from '../utils/logger.js';

const log = createLogger('scraper:casa');

const BASE = 'https://raw.githubusercontent.com/casa-ls/proxy-list/main';

const SOURCES: Array<{ url: string; protocol: ProxyProtocol }> = [
  { url: `${BASE}/http`, protocol: 'http' },
  { url: `${BASE}/socks4`, protocol: 'socks4' },
  { url: `${BASE}/socks5`, protocol: 'socks5' },
];
  ...
  for (const { url, protocol } of SOURCES) {
    try {
      ...
      for (const line of lines) {
        ...
        results.push({ host: host.toLowerCase().trim(), port, protocol, source: 'casa' });
      }
    } catch (err) {
