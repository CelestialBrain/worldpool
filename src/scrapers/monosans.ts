// ─── Monosans Fetcher ─────────────────────────────────────────────────────────
// Fetches proxies from monosans/proxy-list GitHub raw files (updated hourly).

import axios from 'axios';
import type { RawProxy, ProxyProtocol } from '../types.js';
import { createLogger } from '../utils/logger.js';

const log = createLogger('scraper:monosans');

const BASE = 'https://raw.githubusercontent.com/monosans/proxy-list/main/proxies';
  ...
  for (const { url, protocol } of SOURCES) {
    try {
      ...
      for (const line of lines) {
        ...
        results.push({ host: host.toLowerCase().trim(), port, protocol, source: 'monosans' });
      }
    } catch (err) {
